// JS OK roles2 — admin caché + équipes protégées + synchro Firebase
(function(){
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    // ---------- Local storage
    var STORAGE_KEY='tournoi_amis_roles2';
    ...
    function warnStorage(){ var el=id('storage-warning'); if(el){ el.style.display='block'; el.textContent='JS OK roles2' + (MEMORY_ONLY?' — ⚠️ stockage local indisponible':''); } }
    function banner(msg){ var el=id('storage-warning'); if(!el) return; el.style.display='block'; el.textContent=msg; }

    // ---------- State + session
    var state = loadLocal() || { version:11, teams:[], matches:[], locked:false, createdAt:new Date().toISOString(), protect:{teamPassHash:{}} };
    var ui = { open:{}, h2h:false };
    var session = { admin:false, claims:{} }; // claims[teamId]=true sur cet appareil
  ...
  try{
    ...
    function joinCloud(code){
      ...
      cloud.enabled=true; setCloud('connexion…'); loadSession();
      ...
      cloud.ref.on('value', function(snap){
        ...
        state=newState; saveLocal(); renderAll(); setCloud('connecté ('+cloud.id+')');
      });

      pushCloud(true);
      setCloud('connecté ('+cloud.id+')');
      try{ history.replaceState(null,'', location.pathname+'?v=roles2&id='+encodeURIComponent(cloud.id)); }catch(_){}
    }
    ...
    // SHA-256 (avec fallback simple si l'API n'existe pas)
    async function sha256(str){
      ...
      // Fallback non-crypto (suffisant pour notre usage loisir)
      let h=2166136261>>>0;
    ...
    // ---------- Cloud UI
    id('btn-cloud-join').addEventListener('click', ()=> joinCloud(id('cloud-id').value.trim()));
    id('btn-cloud-leave').addEventListener('click', ()=> leaveCloud());
    id('btn-cloud-copy').addEventListener('click', ()=>{
      var code=id('cloud-id').value.trim(); if(!code){ alert('Renseigne d’abord le code tournoi.'); return; }
      var url=location.origin+location.pathname+'?v=roles2&id='+encodeURIComponent(code);
      navigator.clipboard && navigator.clipboard.writeText(url);
      alert('Lien copié !\n'+url);
    });

    // ---------- Admin ON/OFF (PIN jamais affiché)
    id('btn-admin-on').addEventListener('click', ()=>{
    ...
    id('btn-admin-off').addEventListener('click', ()=>{ session.admin=false; saveSession(); renderAll(); });

    // ---------- Teams
    var teamListEl=id('team-list');

    id('btn-create-my-team').addEventListener('click', async ()=>{
      ...
      var idt=uid();
      state.teams.push({id:idt,name,p1,p2});
      if(!state.protect) state.protect={teamPassHash:{}}; state.protect.teamPassHash[idt]=hash;
      session.claims[idt]=true; saveSession();
      saveState(); renderTeams(); updateCounts(); updateLockUI();
      alert('Équipe créée et protégée. Conservez le mot de passe !');
    });

    id('btn-add-team').addEventListener('click', ()=>{
    ...
    id('btn-generate').addEventListener('click', ()=>{
    ...
    function renderTeams(){
      ...
      state.teams.forEach((t,idx)=>{
        ...
        var protectBtns = ''
          + (iOwn || admin ? '<button type="button" class="btn small" data-act="setpass" data-id="'+t.id+'">Définir / changer mot de passe</button>' : '')
          + (!iOwn ? '<button type="button" class="btn small" data-act="login" data-id="'+t.id+'">Se connecter à cette équipe</button>' : '<button type="button" class="btn small" data-act="logout" data-id="'+t.id+'">Se déconnecter</button>');
      ...
      qsa('#team-list [data-act="login"]').forEach(btn=>{
        btn.addEventListener('click', async ()=>{
          var tid=btn.getAttribute('data-id'); var pass=prompt('Mot de passe de l’équipe '+teamName(tid)+' :'); if(!pass) return;
          var hash=await sha256(salt()+'|'+pass);
          var ok=state.protect && state.protect.teamPassHash && state.protect.teamPassHash[tid]===hash;
          if(ok){ session.claims[tid]=true; saveSession(); renderTeams(); renderMatches(); alert('Connexion à l’équipe réussie.'); }
  ...
  saveState();
      ...
      rounds.forEach(r=>{
        ...
        groups[r].forEach(m=>{
          ...
          el.innerHTML=''
            ...
            +'<div class="help">'+(canEdit?'':'Lecture seule — connectez-vous à une des deux équipes.')+'</div>'
          ...
          el.querySelector('[data-expand]').addEventListener('click', ()=>{
            ...
            el.setAttribute('aria-expanded', isOpen?'false':'true'); ui.open[m.id]=!isOpen;
          });
          qsaIn(el,'select[data-match][data-kind]').forEach(sel=>{
            if(!canEdit) sel.disabled=true;
            sel.addEventListener('change', ()=>{
              ...
              var mm=findMatch(m.id); mm[k][ii]=v; saveState();
            });
          });
          qsaIn(el,'input[data-ping]').forEach(inp=>{
            ...
            inp.addEventListener('input', ()=>{
              ...
              var mm=findMatch(m.id); mm.pingPts[ii][w]=v; saveState();
            });
          ...
          qsaIn(el,'input[data-palet]').forEach(inp=>{
            ...
            inp.addEventListener('input', ()=>{
              if(!canEdit) return;
              var w=inp.getAttribute('data-palet'); var v=inp.value===''?null:clampInt(parseInt(inp.value,10),0,11);
              var mm=findMatch(m.id); mm.palet[w]=v; saveState();
            });
            inp.addEventListener('keydown', e=>{ if(e.key==='Enter'){ e.preventDefault(); inp.blur(); } });
          });
          var clr=el.querySelector('[data-clear]'); if(clr) clr.addEventListener('click', ()=>{ if(!canEdit) return; clearMatch(m.id); });
    ...
    function computeSetWins(m){
      ...
      m.darts.forEach(v=>{ if(v===0) aw.darts++; else if(v===1) bw.darts++; });
      getPingPts(m).forEach(s=>{ if(isPingValid(s.a,s.b)){ if(s.a>s.b) aw.ping++; else if(s.b>s.a) bw.ping++; } });
      return {aw:aw, bw:bw};
    }
    ...
    function computeLeaderboard(){
      ...
      state.matches.forEach(m=>{
        ...
        m.darts.forEach(v=>{ if(v===0){A.dartsW++;A.points+=5;} else if(v===1){B.dartsW++;B.points+=5;} });
    ...
    function pointsForTeamInMatch(m,teamId){
      ...
      var pts=0;
      m.darts.forEach(v=>{ if(v===0&&isA) pts+=5; else if(v===1&&isB) pts+=5; });
    ...
    function renderH2H(){
      ...
      teams.forEach(ti=>{
        ...
        teams.forEach(tj=>{
          ...
          td.className='h2h-clickable'; td.dataset.matchId=m.id;
          tr.appendChild(td);
        });
        tbody.appendChild(tr);
      });
    ...
    function goToMatch(mid){
      var tab=qs('.tab[data-tab="calendrier"]'); tab&&tab.click && tab.click();
      ui.open[mid]=true;
      setTimeout(()=>{ var card=qs('.match-card[data-id="'+mid+'"]'); if(card){ card.setAttribute('aria-expanded','true'); try{ card.scrollIntoView({behavior:'smooth',block:'start'}); }catch(_){ card.scrollIntoView(); } } },0);
    }

    // ---------- Export/Import/Share
    id('btn-export').addEventListener('click', ()=>{
      var data=JSON.stringify(state,null,2); var blob=new Blob([data],{type:'application/json'});
      var url=URL.createObjectURL(blob); var a=document.createElement('a'); a.href=url; a.download='tournoi-amis-'+new Date().toISOString().slice(0,10)+'.json'; a.click(); URL.revokeObjectURL(url);
    });
    var importFile=null; id('file-import').addEventListener('change', e=>importFile=e.target.files[0]);
    id('btn-import').addEventListener('click', ()=>{
      if(!importFile){ alert('Sélectionnez un fichier JSON.'); return; }
      importFile.text().then(text=>{
        try{
          var data=JSON.parse(text);
          if(!(data && Array.isArray(data.teams) && Array.isArray(data.matches))) throw new Error('format');
          state=data; if(!state.protect) state.protect={teamPassHash:{}}; saveState(); renderAll(); alert('Import réussi !');
        }catch(e){ alert('Fichier invalide.'); }
      });
    });
    id('btn-share').addEventListener('click', ()=>{
      var json=JSON.stringify(state), b64=btoa(unescape(encodeURIComponent(json))), enc=encodeURIComponent(b64);
      var url=location.origin+location.pathname+'?v=roles2#s='+enc;
      var inp=id('share-url'); inp.value=url; inp.select(); document.execCommand && document.execCommand('copy');
      navigator.clipboard && navigator.clipboard.writeText(url);
      alert('Lien (offline) copié !');
    });
    (function(){
      var m=location.hash.match(/^#s=([^&]+)$/); if(!m) return;
      try{
        var b64=decodeURIComponent(m[1]); var json=decodeURIComponent(escape(atob(b64))); var data=JSON.parse(json);
        if(!(data && Array.isArray(data.teams) && Array.isArray(data.matches))) throw new Error('format');
        state=data; if(!state.protect) state.protect={teamPassHash:{}}; saveLocal(); history.replaceState(null,'',location.pathname+location.search);
      }catch(_){ alert('Lien de partage invalide.'); }
    })();
    ...
    id('btn-reset').addEventListener('click', ()=>{
      ...
      state={version:11,teams:[],matches:[],locked:false,createdAt:new Date().toISOString(),protect:{teamPassHash:{}}};
      session={admin:false,claims:{}}; saveSession(); saveState(); renderAll();
    });
    id('btn-unlock').addEventListener('click', ()=>{
      if(!isAdmin()){ alert('Réservé à l’admin.'); return; }
      if(!confirm('Déverrouiller le calendrier ?')) return;
      state.locked=false; saveState(); renderTeams(); renderMatches(); updateLockUI();
    });

    // ---------- Helpers communs
    function findMatch(idv){ return state.matches.find(x=>x.id===idv); }
    function clearMatch(idv){ var m=findMatch(idv); if(!m||!canEditMatch(m)) return; if(!confirm('Effacer tous les scores de ce match ?')) return; m.darts=[null,null,null]; m.pingPts=[{a:null,b:null},{a:null,b:null},{a:null,b:null}]; m.palet={a:null,b:null}; saveState(); renderMatches(); renderLeaderboard(); renderH2H(); }
    ...
    // ---------- Save + cloud push
    function saveState(){ saveLocal(); renderLeaderboard(); cloud.enabled && pushCloud(false); }

    // ---------- Init
    loadSession(); renderAll(); warnStorage(); showH2H(ui.h2h); setCloud(cloud.enabled?'connecté':'hors ligne');
    ...
    id('btn-toggle-h2h').addEventListener('click', e=>{ e.preventDefault(); showH2H(!ui.h2h); });
