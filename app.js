...
(function(){
  ...
  function showBanner(extra){
    ...
    if(msg.indexOf('JS OK') === -1){ el.textContent = (msg?msg+' ':'') + 'JS OK final1'; }
    if(extra){ el.textContent += ' ' + extra; }
  }
  showBanner();

  // --- Storage ---
  var STORAGE_KEY='tournoi_amis_final1';
  ...
  function updateStorageWarning(){
    ...
    if(el){
      ...
      el.textContent = (msg?msg+' ':'') + 'JS OK final1';
    }
  }
  ...
  function qsaIn(node,sel){ return Array.prototype.slice.call(node.querySelectorAll(sel)); }
  ...
  function renderTeams(){
    ...
    if(state.teams.length===0){
      teamListEl.appendChild(help('Aucune équipe pour le moment. Ajoutez votre première équipe !'));
    ...
    qsa('#team-list input[data-field]').forEach(function(inp){
      inp.addEventListener('input', function(){
        ...
        t[field]=inp.value; saveState(); renderMatches(); renderLeaderboard();
      });
    });
  ...
  // --- Génération du calendrier (round-robin équilibré) ---
    ...
    rounds.forEach(function(r){
      ...
      groups[r].forEach(function(mm){
        ...
        el.innerHTML=''
          ...
          +    '<div><button type="button" class="btn small" data-clear="'+mm.id+'">Effacer ce match</button></div>'
          +  '</div>'
          +'</div>';
        ...
        qsaIn(el,'select[data-match][data-kind]').forEach(function(sel){
          sel.addEventListener('change', function(){
            ...
            var match=findMatch(mm.id); match[kind][idx]=val; saveState();
          });
        });
        // ping
        qsaIn(el,'input[data-ping]').forEach(function(inp){
          inp.addEventListener('input', function(){
            ...
            match.pingPts[idx][which]=val; saveState();
          });
        ...
        qsaIn(el,'input[data-palet]').forEach(function(inp){
          inp.addEventListener('input', function(){
            ...
            var match=findMatch(mm.id); match.palet[which]=val; saveState();
          });
          inp.addEventListener('keydown', function(e){ if(e.key==='Enter'){ e.preventDefault(); inp.blur(); } });
        });
        // clear
  ...
  function renderPing(m){
    ...
    for(var i=0;i<3;i++){
      ...
      html += ''
        ...
        +  '<div><label>Ping — '+labels[i]+' — '+esc(names[0])+'</label><input type="number" min="0" max="99" step="1" value="'+(s.a==null?'':s.a)+'" data-ping="a" data-index="'+i+'"/></div>'
        +  '<div><label>Ping — '+labels[i]+' — '+esc(names[1])+'</label><input type="number" min="0" max="99" step="1" value="'+(s.b==null?'':s.b)+'" data-ping="b" data-index="'+i+'"/></div>'
